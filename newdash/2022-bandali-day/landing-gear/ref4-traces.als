...
pred dsh_initial [s: one DshSnapshot] {
  ...
  (s . LandingGear_extend_gears_electro_valve) = False and
  (s . LandingGear_anomaly) = False
}
...
pred LandingGear_health_monitoring_pre [s: one DshSnapshot] {
  some (LandingGear & (s . dsh_conf0))
  ! (LandingGear in (s . dsh_sc_used0))
}


pred LandingGear_health_monitoring_post [s: one DshSnapshot, sn: one DshSnapshot] {
  (sn . dsh_conf0) =
  (((s . dsh_conf0) - LandingGear) + LandingGear)
...
pred LandingGear_health_monitoring [s: one DshSnapshot, sn: one DshSnapshot] {
  s . LandingGear_health_monitoring_pre
